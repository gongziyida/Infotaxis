...
class Infotaxis:
	def __init__(self, d, r, a, v, winddir, tau, dt, dim, pos, src_pos, src_radius, window):
		...
		--------
		d: diffusivity (m**2/s)
		r: emission rate (particles/s)
		a: size of the searcher (m)
		v: wind speed (m/s)
		winddir: wind direction, between 0 (East) and 2*PI, counterclockwise
		tau: particle lifetime (s)
		dim: dimension, either 2 or 3
		src_pos: the posiiton of the source, i.e. r0 in the paper. The first entry is y 
			and the second is x
		"""
	...
	def concentration_map(self):
		"""
		Return the concentrations at the current position given source positions
		"""
	...
	def update_posterior(self, hit_rates, num_hits, inplace=False):
		...
		Patameter
		--------
		num_hits: the number of hits encountered at t. It should between 0 (no hit)
			and 1 (hits)
		"""
